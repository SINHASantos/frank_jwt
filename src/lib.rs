/**
* Copyright (c) 2015-2025 Alex Maslakov
*
...
fn get_signing_input(payload: &JsonValue, header: &JsonValue) -> Result<String, Error> {
    ...
    let encoded_header = general_purpose::URL_SAFE_NO_PAD.encode(header_json_str.as_bytes());
    let payload_json_str = serde_json::to_string(payload)?;
    let encoded_payload = general_purpose::URL_SAFE_NO_PAD.encode(payload_json_str.as_bytes());
...
fn sign_hmac<P: ToKey>(data: &str, key_path: &P, algorithm: Algorithm) -> Result<String, Error> {
    ...
    Ok(general_purpose::URL_SAFE_NO_PAD.encode(hmac.as_slice()))
}
...
fn sign_es<P: ToKey>(
    ...
    Ok(general_purpose::URL_SAFE_NO_PAD.encode(&signature[0..length]))
}
...
fn sign(data: &str, private_key: PKey<Private>, digest: MessageDigest) -> Result<String, Error> {
    ...
    Ok(general_purpose::URL_SAFE_NO_PAD.encode(signature.as_slice()))
}
...
fn decode_segments(encoded_token: &str) -> Result<(JsonValue, JsonValue, Vec<u8>, String), Error> {
    ...
    let signature = general_purpose::URL_SAFE_NO_PAD.decode(crypto_segment.as_bytes())?;
...
fn decode_header_and_payload(
    ...
    let b64_to_json = |seg| -> Result<JsonValue, Error> {
        serde_json::from_slice(general_purpose::URL_SAFE_NO_PAD.decode(seg)?.as_slice())
            .map_err(Error::from)
    };
...
mod tests {
    ...
    use super::{
        Algorithm, STANDARD_HEADER_TYPE, ValidationOptions, decode, encode, secure_compare,
        validate_signature, verify_expiration,
    };
    ...
    fn test_encoded_validate_signature_jwt_rs256_invalid() {
        ...
        // assert!(!maybe_res.unwrap());
        match maybe_res {
            Ok(valid) => assert!(!valid, "Expected signature to be invalid, but it was accepted"),
            Err(e) => panic!("validate_signature failed with error: {:?}", e),
        }
    }
    ...
    fn test_validate_signature_jwt_ec256() {
        ...
        let maybe_valid_sign1 =
            validate_signature(jwt1, &get_ec_public_key_path(), Algorithm::ES256);
        assert!(maybe_valid_sign1.unwrap());

        let jwt2 = encode(header, &get_ec_private_key_path(), &p1, Algorithm::ES256).unwrap();
        let maybe_valid_sign2 =
            validate_signature(&jwt2, &get_ec_public_key_path(), Algorithm::ES256);
        assert!(maybe_valid_sign2.unwrap());
    }
    ...
    fn test_validate_signature_jwt_ec256_invalid() {
        ...
        let maybe_valid_sign =
            validate_signature(&jwt1, &get_bad_ec_public_key_path(), Algorithm::ES256);
